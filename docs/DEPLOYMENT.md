...
1. **Caller → server:** `Authorization: Bearer <api-key>` shared secret. Enforced by `APIKeyMiddleware` (`src/gdrive_mcp/api_key_middleware.py`). Constant-time comparison via `hmac.compare_digest`. Missing/wrong key → `401 {"error": "unauthorized"}`.
